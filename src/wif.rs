...
mod tests {
    ...
    fn test_wif_checksum_valid() {
        ...
        assert_eq!(&payload[1..33], key.as_slice(), "bytes 1-32 must be the private key");
        assert_eq!(payload[33], 0x01, "last payload byte must be 0x01 (compressed flag)");
    }
}
